...
Given a quality function $Q$, a relation $R$, and a language $L$, find valid program $p \in L$ that optimizes the quality function.
\ewu{This used to minimize Q.  I changed to maximize based on above text.  Make sure we are consistent throughout the paper}
\[
\textsf{opt}(Q,R,L) = ~ \max_{p \in L} Q( l(R) ).  
\]
\end{problem}
