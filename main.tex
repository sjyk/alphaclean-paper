...
\title{AlphaClean: Automatic Data Cleaning With Distributed Search and Machine Learning}
...
This paper explores an analogous approach to data cleaning, a cost-based search over data transformations.
We propose \sys where users articulate high level cleaning goals (as a function that measures the quality of the data), and the system automatically finds an interpretable cleaning program to best maximize that objective.
\sys is very general and can express a very large class of data cleaning problems including cleaning with integrity constraints and statistical models.
The system additionally adaptively learns a search heuristic with Machine Learning to exploit systematic errors in the dataset.
With \sys: (1) there are few restrictions on what the user can express at the cost of run time, (2) the output is a sequence of transformations rather than a database instance so the output can apply to future data, and (3) the optimization algorithm is parallelizable.
We compared \sys to several special case integrity constraint systems and quantitative data cleaning approaches, and suprisingly, found that we can match the accuracy and the runtime by leveraging parallelism.
\end{abstract}
