...
\begin{abstract}
Recent results in AI such as AlphaGo have shown that a combination of machine learning and distributed search can effectively optimize very complex objective functions, e.g., the value of board positions in a complex Go game. This paper proposes an analogous approach to data cleaning:  optimizing a distributed search over data transformations.  We present \sys where a user specifies an objective function and the distributed system searches for an interpretable cleaning program (sequence of transformations) that maximizes the objective.  In contrast to special-purpose data cleaning methods, \sys is general in that one can express a large class of objectives including cleaning with integrity constraints and statistical models.  Although \sys does not currently incorporate Deep Learning, it is similar to AlphaGo in that it adaptively learns a search heuristic with a logistic regression classifier to discover systematic patterns and is efficiently parallelizable. \sys provides 3 benefits: (1) it is general vs. special case, (2) it is well-suited for cluster and cloud computing, (3) the output transform sequence is lightweight and can be easily applied as new data is added. We compare \sys to several special-purpose systems on data cleaning benchmarks and find that \sys achieves similar accuracy and runtimes using the unified and general framework.
\end{abstract}
