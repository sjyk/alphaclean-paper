...
\stitle{Terrorism} The Global Terrorism Database~\cite{data-terrorism} contains terrorist attacks scraped from news sources.  Attributes include date, location, text details, and the number of fatalities and injuries.  Three types of errors need to be cleaned---Entity Matching for duplicates of the same attack; outliers to resolve missing fatality and injury values that are encoded as $0$; and functional dependencies for inconsistent location values.
There are around $200$ records since $1970$.
We downloaded this dataset and sought to understand whether terrorist attacks have become more lethal than they were in the 1970s.  We hand-cleaned the records for ground truth.

%to create a gold standard.  It turns out that, in this dataset, attacks have become more lethal, but fewer in number than 50 years ago.  This task was intentionally open-ended to represent the nature of the iterative analysis process.
...
We use the FEC setup to study details of \sys.
