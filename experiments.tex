...
In the next experiment, we consider a dataset that contains many different error types.
The Global Terrorism Database is a dataset of terrorist attacks scraped from news sources since 1970.
Each record contains the date, location, details about the attack, and the number of fatalities and injuries.

This dataset contains a number of inconsistencies that make analysis difficult.
(1) It has a substantial number of duplicates, where records that refer to the same terrorist incident are marked as separate records.
(2) A number of missing values in the fatalities and injuries numbers in the dataset are encoded as zeros.
(3) There are inconsistencies in the location information.
